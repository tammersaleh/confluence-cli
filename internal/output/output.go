...
func (p *Printer) PrintItem(v any) error {
	...
	// Filter first, then enrich: enrichment fields (`created_at_iso`,
	// `ts_iso`, and any keys added by EnrichFunc) are always attached to
	// surviving source fields, so `--fields=ts` keeps the resolved `ts_iso` too.
