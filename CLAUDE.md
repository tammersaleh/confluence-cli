...
`page move`, `page delete`, `page convert-to-live`,
...
internal/
  cli/
    ...
    page.go                    `page list/get/children/descendants/ancestors/tree` (reads) + `page create/update/move/delete/convert-to-live` (writes). Derives site/space from --space or URL args; --body-format incl. derived markdown on read. `page get` emits `status`, `parent_id`/`parent_type` when present, and `subtype` when the page is a live doc. `page tree` builds the hierarchy in-command, sorted by ID (not display order). `page update` preflights GetPage as ADF to enforce --if-version (version_conflict) and preserve body as EXACT ADF (not a storage round-trip) + status when not supplied; rejects no-op (no title+no body) and a title-unchanged update (updated:false, no PUT). Only a body-piped `page update` runs guardInlineComments (last precondition before PUT): fail-closed refusal (`unresolved_inline_comments`) if any inline comment != "resolved", overridable with --allow-unresolved-inline-comments (skips the inspection entirely). Title-only update and same-space `page move` re-send exact ADF (live-verified to preserve anchors) so they are NOT guarded. `page move` reparents within a space via UpdatePage (ParentID *string, exact ADF re-send, version bump) or across spaces via the v1 MovePage endpoint (body untouched, NO version bump, --if-version preflight-only); symmetric two-ref resolution; refuses self-parent/cycle (`invalid_move`), no-op when already child (moved:false). `page create --live` sets subtype=live (supported REST). `page convert-to-live` calls the UNDOCUMENTED internal /cgraphql endpoint (convertPageToLiveEditAction) - no public API exists; warns once on stderr, one site per invocation, inline per-item errors.
    ...
    comment.go                 `comment list` (footer + inline; inline rows carry resolution_status/original_selection/inline_marker_ref; --resolution-status filters inline server-side, implies --inline, conflicts with --footer; --replies drains reply threads recursively with parent_id) + `comment add` (footer, or inline via --inline --selection-text with --match-index (0-based)/--match-count).
  ...
  confluence/
    ...
    types.go                   API response types: Space, Page, PageContent, Attachment. Client interface (incl. MovePage, the v1 move endpoint).
...
- `confluence page get <id|url>...` - fetch pages by id or URL (one site per invocation). `--body-format storage|atlas_doc_format (adf)|view|markdown (md)`, default `storage`. ADF `body` is a nested object; `markdown` is derived from storage (attachments resolved to remote URLs) and adds `source_body_format`. Rows carry `status`, `parent_id`/`parent_type` when the page has a parent, and `subtype` only for live docs (omitted for regular pages). `--resolve-authors` adds an `author_name` sibling (best-effort, cached user lookups). Per-item errors go inline and bump `_meta.error_count`.
...
- `confluence page update <id|url> --if-version <n> [--title <t>] [--body-format storage|adf|markdown] [--allow-unresolved-inline-comments]` - update under optimistic concurrency; body on stdin. A version mismatch (preflight OR a PUT-time 409) fails with `version_conflict` and writes nothing. No title and no body is `invalid_input`; a title-unchanged no-op returns `updated:false` with no PUT. Title-only preserves body as EXACT ADF (and status); body-only preserves title. When a NEW body is piped, refuses with `unresolved_inline_comments` (fail-closed, `data` carries the blocking comments) unless `--allow-unresolved-inline-comments`; a title-only update is not guarded (exact-ADF passthrough preserves anchors). Row: `id`, `title`, `version`, `previous_version`, `updated`, `web_url`.
- `confluence page move <id|url> --parent <id|url> --if-version <n>` - move a page under a new parent in the same space or another space (preserves id/history/comments/attachments). Symmetric two-ref site resolution. Refuses self-parent and cycles (`invalid_move`). Already-child is `moved:false` with no write. Same-space: v2 PUT re-sending exact ADF (NOT inline-comment-guarded), version bumps, PUT-time `version_conflict` as in `page update`. Cross-space: v1 `MovePage` (`/wiki/rest/api/content/{id}/move/append/{target}`, the only public API; v2 refuses "Only DRAFT pages can be moved between spaces"), body untouched, version does NOT bump, `--if-version` preflight-only; restrictions unverified. Row: `id`, `title`, `previous_space_id`, `space_id`, `previous_parent_id`, `parent_id`, `previous_version`, `version`, `moved`, `web_url`.
...
- `confluence comment list <page id|url>` - a page's comments. Drains footer + inline (no cursor); `--footer`/`--inline` narrow to one kind. `--resolution-status open|reopened|resolved|dangling` filters inline server-side (implies `--inline`, conflicts with `--footer`). `--replies` recursively drains reply threads, emitting each reply after its parent with a `parent_id`. `--resolve-authors` adds an `author_name` sibling (best-effort, cached). Rows carry `id`, `kind` (`footer`/`inline`), `body`, `author_id`, `created_at`, `web_url`; inline rows also carry `resolution_status`, `original_selection`, `inline_marker_ref`. A missing page is a fatal `page_not_found`.
