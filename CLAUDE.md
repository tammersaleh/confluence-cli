...
- `confluence page move <id|url> --parent <id|url> --if-version <n>` - reparent a page within its space (preserves id/history/comments/attachments). Symmetric two-ref site resolution. Refuses cross-space (`cross_space_move_unsupported`, `data` carries source/dest spaces+urls; do it in the UI), self-parent and cycles (`invalid_move`). Already-child is `moved:false` with no PUT. Same `version_conflict` behavior as `page update`; re-sends exact ADF so it is NOT inline-comment-guarded. Row: `id`, `title`, `space_id`, `previous_parent_id`, `parent_id`, `previous_version`, `version`, `moved`, `web_url`.
